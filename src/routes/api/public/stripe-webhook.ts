...
async function buildWelcomeHtml(level: string, tier: string, loginLink: string): Promise<string> {
  const { emailShell } = await import("@/lib/email-shell.server");
...
async function sendWelcomeEmail(
  ...
  const html = await buildWelcomeHtml(level, tier, loginLink);
